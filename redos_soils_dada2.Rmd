...
```{r data_path}
demux.dir = file.path("Data/redos_soils")
scratch.dir = file.path("dada2")
map.file = file.path("Data/GoMRI_mapfile.csv")
# make directory for output
ps.rds = file.path(scratch.dir, "redos_soils.rds")
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft=15, truncLen=c(225,175),
...
```{r seqtab}
seqtab_redos_soils <- makeSequenceTable(mergers)
```

How many samples are there?  How many ASVs?
```{r}
dim(seqtab_redos_soils)
```

Let's check the size distribution of the ASVs we have inferred. 
```{r}
table(nchar(getSequences(seqtab_redos_soils)))
```

This removes all sequences that don't fall between ___ bp in length
```{r}
seqtab2_redos_soils=seqtab_redos_soils[,nchar(colnames(seqtab_redos_soils)) %in% seq(276,295)]
```

```{r}
dim(seqtab2_redos_soils)
```
...
seqtab_redos_soils.nochim <- removeBimeraDenovo(seqtab2_redos_soils, method="consensus", multithread=TRUE, verbose=TRUE)
dim(seqtab_redos_soils.nochim)
sum(seqtab_redos_soils.nochim)/sum(seqtab2_redos_soils)
```
...
track <- cbind(out, sapply(dadaFs, getN), sapply(dadaRs, getN), sapply(mergers, getN), rowSums(seqtab_redos_soils.nochim))
...
```{r taxify}
taxa_redos_soils <- assignTaxonomy(seqtab_redos_soils.nochim, silva.ref, multithread=TRUE)
taxa_redos_soils.print <- taxa_alex_soils # Removing sequence rownames for display only
rownames(taxa_redos_soils.print) <- NULL
head(taxa_redos_soils.print)
```


##Save seq table and tax table as rds
```{r}
saveRDS(seqtab_redos_soils.nochim, file = "seqtab_redos_soils.Rds")
saveRDS(taxa_redos_soils, file = "taxatab_redos_soils.Rds")
```
...
```{r}
taxtab_redos_soils = readRDS("taxatab_redos_soils.Rds")
seqtab_redos_soils = readRDS("seqtab_redos_soils.Rds")
samtab_redos_soils = read_tsv("Data/mapfile_redos_soils.txt")
map_2 = file.path("Data/mapfile_redos_soils.txt")
```
...
```{r load_map}
meta.df.2 = read_tsv(map_2, comment= "#q2")
meta.df.2 = meta.df.2[1:96,]
colnames(meta.df.2)[colnames(meta.df.2) == 'SampleID'] <- 'SampleID'
meta.df.2 = meta.df.2 %>%
...
```{r make-phyloseq}
otus.2 = otu_table(seqtab_redos_soils, taxa_are_rows=FALSE)
sd.2 = sample_data(meta.df.2)
tax.2 = tax_table(taxtab_redos_soils)
ps_redos_soils <- phyloseq(otus.2,
               sd.2,
               tax.2)
```
...
```{r}
saveRDS(ps_redos_soils, file = "phyloseq/ps_redos_soils.rds")
```

## Check Phyloseq RDS

```{r}
loaded.ps = readRDS("phyloseq/ps_redos_soils.rds")
print(loaded.ps)
```
